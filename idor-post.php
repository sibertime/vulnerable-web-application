...
<div class="w3-main" style="margin-left:250px">
    ...
    <div class="w3-container w3-black w3-padding-32">
        ...
        if (!isset($_SESSION["usernameForIdor"])) {
            ...
            <header style="background-color: whitesmoke;color: black;margin-bottom: 15px;"
                ...
                <p class="w3-right">
                    BÜSİBER Sosyal Ağ Platformu
                </p>
            </header>
            <?php
            $postQuery = "SELECT * FROM posts WHERE post_id = :post_id order by post_id desc limit 1";
            ...
            if ($row) {
                ...
                $postCommentQuery = "SELECT * FROM post_comments WHERE post_id = :post_id and post_comment_status= '1' order by post_comment_id desc";
                $postCommentStatement = $connect->prepare($postCommentQuery);
                $postCommentStatement->execute(
                    array(
                        'post_id' => $post_id
                    )
                );
                $postCommentCount = $postCommentStatement->rowCount();
                $postCommentResult = $postCommentStatement->fetchAll(\PDO::FETCH_ASSOC);
                foreach ($postCommentResult as $postCommentRow) {
                    echo "<div style='background-color: whitesmoke;color: black;padding-left: 6px;padding-top: 3px;padding-bottom: 3px;margin-bottom: 6px'> Gönderen : " . $postCommentRow['post_comment_user'] . " | Tarih : " . $postCommentRow['post_comment_date'] . " <br> " .
                        "<b>" . htmlspecialchars($postCommentRow['post_comment']) . "</b><br>";
                    echo $_SESSION['usernameForIdor'] == $postCommentRow['post_comment_user'] ? "<a style='color: red' href='idor-delete-comment.php?id=" . $postCommentRow['post_comment_id'] . "'>Yorumu Sil</a>" : '';
                    echo "</div>";
                }

                echo "<h3>Yorum Yap</h3>";
                echo "<form method='POST'>";
                echo "<input name='postContent' class='w3-input w3-border' type='text' placeholder='' style='width:100%'/>";
                echo "<br><input type='submit' value='Yorum Yap' class='w3-button w3-red w3-margin-bottom'/>";
                echo "</form>";
            } else {
                echo '<script>window.location = "idor.php"</script>';
            }
    ...
    if (isset($_POST['postContent'])) {
        ...
        $insertQuery = $connect->prepare("INSERT INTO post_comments SET post_id = ?, post_comment = ?, post_comment_user = ?");

        $insert = $insertQuery->execute(array(
            $post_id, $content, $_SESSION['usernameForIdor']
        ));

        if ($insert) {
            $url = "?post_id=" . $post_id;
            echo "<script>window.location = 'idor-post.php$url'</script>";
        }
    }
    ?>
</div>
